...
export default function CarteMembresPage() {
  ...
  const filteredMembers = members.filter((member) => {
    ...
    const matchesSearch =
      ...
      member.experience.toLowerCase().includes(searchLower);

    // Filtres
    const matchesActivite = !filters.activite || member.profession.toLowerCase().includes(filters.activite.toLowerCase());
    const matchesPays = !filters.pays || member.country.toLowerCase().includes(filters.pays.toLowerCase());
    const matchesCertified = !filters.certifiedOnly || member.isVerified;
    const matchesPremium = !filters.premiumOnly || member.isPremium;

    return matchesSearch && matchesActivite && matchesPays && matchesCertified && matchesPremium;
  });

  // Tri des membres filtrés
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sortedMembers.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-12">
                    <svg className="mx-auto h-16 w-16 text-neutral-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <p className="text-lg font-medium text-neutral-400 mb-2">Aucun membre trouvé</p>
